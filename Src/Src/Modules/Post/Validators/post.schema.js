...
export const createPostSchema = Joi.object().keys({
  ...
  visibility: Joi.string().valid("Public", "Private", "FriendsOnly", "SpecificUsers").default("Public").messages({
    "string.valid": "Visibility must be 'Public', 'Private', 'FriendsOnly', or 'SpecificUsers'",
  }),
  specificUsers: Joi.array().items(Joi.string().hex().length(24)).optional().messages({
    "array.base": "SpecificUsers must be an array",
    "string.hex": "User IDs must be valid MongoDB ObjectIds",
    "string.length": "User IDs must be 24 characters long",
  }),
...
export const updatePostSchema = Joi.object().keys({
  ...
  visibility: Joi.string().valid("Public", "Private", "FriendsOnly", "SpecificUsers").optional().messages({
    "string.valid": "Visibility must be 'Public', 'Private', 'FriendsOnly', or 'SpecificUsers'",
  }),
  specificUsers: Joi.array().items(Joi.string().hex().length(24)).optional().messages({
    "array.base": "SpecificUsers must be an array",
